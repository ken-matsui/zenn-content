...
SPA ではない場合や、そもそもフロントエンドでない場合は、Firebase Hosting を使用できません。しかし、Cloud Run を使用すれば、そういった場合でもプレビューデプロイを実現することができます。そちらに関する詳細は、[プレビューデプロイをCloud Runで行う](/articles/preview-deploy-on-cloud-run)にて紹介しています。
